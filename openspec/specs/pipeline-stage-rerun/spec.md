...
The system SHALL allow re-running the full ASR pipeline (extract → transcribe → split → initialize subtitles → build speaker reference bank) for a project that already has a stored `source.*` video, without requiring a new upload.

#### Scenario: Successful ASR rerun

- **WHEN** the client calls `POST /api/projects/{id}/rerun/asr` and `source.*` exists
- **THEN** the server returns HTTP 202, clears downstream artifacts (subtitles, segments, references, TTS, remux outputs), enqueues background ASR processing, and project status progresses through `extracting`, `transcribing`, `splitting`, to `ready`
...
- **THEN** the server clears the error, enqueues ASR processing, and status leaves `failed`
